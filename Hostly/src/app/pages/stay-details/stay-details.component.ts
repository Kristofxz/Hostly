...
export class StayDetailsComponent implements OnInit, OnDestroy {
  ...
  async onClickLike(ev: Event) {
    ev.stopPropagation()
    try {
      if (!this.user) this.snackBar.open('Kérlek jelentkezz be ', 'Vissza', { duration: 3000 })
      else {
        ...
        const msg = this.isLikeByUser ? 'Utazás sikeresen hozzáadva a kívánságlistához!' : 'Utazás sikeresen eltávolítva a kívánságlitából!'
        this.snackBar.open(msg, 'Vissza', { duration: 3000 })
      }
    } catch (err) {
      console.log('err:', err)
    }
  ...
  addOrder() {
    ...
    if (!user) this.snackBar.open('Kérlek jelentkezz be először!', 'Vissza', { duration: 3000 })
    else {
